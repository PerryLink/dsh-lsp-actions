...
| Surface | Status |
|---|---|
| Harness | DeepSeek Harness `dsh-v0.1.3-alpha.1`（对 `>=0.1.2-rc.1 <0.2.0` 声明兼容）；插件不自写会话事件——宿主记录标准 tool/call + tool/result 事件；已于 2026-09-06 对照 dsh-v0.1.3-alpha.1 master checkout 核验（完整 gate 链 + profile 安装冒烟）。 |
| Node | `^22.19.0 \|\| >=24.0.0` |
